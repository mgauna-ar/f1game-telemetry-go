...
*   **Data Transformation & Navigation:** Keep frontend payloads lightweight. Only broadcast the specific data points needed by the UI (e.g., extracting `WorldPositionX/Z` for the mini-map) rather than sending raw, unparsed packets. Top navigation bar features a modern segmented layout ordered: 1) **Session History** (default landing view), 2) **Lap Comparator**, 3) **Live Telemetry** (with live pulse badge and dedicated animated "Waiting for Telemetry Data" empty state when no live session packets are present). For historical lap comparison telemetry, server-side LTTB downsampling (`?maxPoints=800`) is used to maintain 60FPS UI performance. Lap Comparator features side-by-side Slot A & Slot B comparison cards with independent Session & Custom Lap Pickers, a **Link Sessions / Cross-Session** toggle (strictly filtered to the same circuit when comparing cross-session), an enhanced **Collapsible Quick Select Driver Leaderboard** (with P1/P2/P3 rank badges, delta to leader, tyre compound badges, sector timing chips, real-time driver search, cross-session filtering tabs, active Slot A/B badges, and toolbar toggle button), rich searchable **Custom Lap Selectors** (with driver filter tabs, tyre compound badges, sector time breakdowns, fastest/personal best badges, and a "Valid Laps Only" toggle), 2-column layout with a sticky Track Map sidebar (smart outward corner offsetting, anti-collision turn badges, view mode switcher, and turn quick-jump ribbon), separated Throttle/Brake charts with driver names, integrated ERS battery % and ERS deploy mode step charts, synchronized track distance zoom buttons, real-time hover point telemetry readouts with contextual turn indication, and an embedded **AI Race Engineer** slide-over drawer with real-time SSE streaming for contextual telemetry analysis and coaching.

...
*   **Storage Invariants:**
    ...
    - **Settings ownership:** Radio engineer settings (thresholds, alert switches, trigger preset) are owned by the server and persisted in the SQLite `settings` table (`engineer_config`), always loaded on top of `engineer.DefaultEngineerConfig()` so new fields keep their defaults. The UI reads defaults from `GET /api/ai/engineer/config/defaults` instead of hard-coding them. Settings every device shares live in `internal/settings` (one JSON document each in the `settings` table) behind `GET`/`PUT /api/settings/{ai,voice,ptt}`: AI provider, model and API keys per provider (keys are write-only and never returned), the engineer's persona and voice, and push-to-talk (applied to the input manager at startup). AI requests are completed in the API layer in the order request, saved setting, env var (`GEMINI_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `LLM_PROVIDER`, `LLM_MODEL`), built-in default, and `internal/ai` owns the default model names. Only per-device preferences (volume, radio effects, alerts on/off, view modes) live in `localStorage`, through `frontend/src/utils/storage.ts`, with every key listed in its `KnownStorageKey` type. The router rejects cross-site writes (`http.CrossOriginProtection`) and sends no CORS headers. Server startup settings come from flags, then environment variables, then an optional `.env` file.

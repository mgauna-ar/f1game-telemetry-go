...
*   **Navigation Layout:** Top nav bar features the `<F1TelemetryLogo />` brand identity, followed by three main sections:
    ...
    **3) Live Session**
    ...
    - **Interactive Voice Race Engineer** (`<LiveRadioHUD />` — floating bottom-left):
        ...
        - **Proactive Telemetry Radio Watcher & Tactical Intelligence** (`useProactiveTelemetryRadio`): Automatically alerts the driver — configurable tyre wear % warnings & punctures, rear tyre thermal overheating (>115°C) & cold tyre warnings, rival DRS threats with compound & damage context, pit stop window / rival undercut attempts, Safety Car / VSC / Red Flag deployments, and rain radar crossover windows.

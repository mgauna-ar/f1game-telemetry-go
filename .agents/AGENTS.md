...
*   **Navigation Layout:** Top nav bar features the `<F1TelemetryLogo />` brand identity, followed by three main sections:
    ...
    **3) Live Session**
    ...
    - **Interactive Voice Race Engineer** (`<LiveRadioHUD />` — floating bottom-left):
        ...
        - **Proactive Telemetry Radio Watcher & Session Intelligence** (`useProactiveTelemetryRadio`): Monitors 8 distinct telemetry subsystems with independent per-category anti-spam cooldowns and instant Emergency Bypass for safety-critical events (punctures, severe wing damage, SC/VSC, Red Flags, penalties, corner-cutting track limits):
          1. *Tyre Wear & Thermal Management:* Wear warning % & critical % sliders, critical puncture bypass (>=95%), thermal surface overheating (>115°C) and cold tyre warnings.
          2. *Aero & Mechanical Damage:* Front wing flap warning & critical (>=40% box) thresholds, floor/diffuser downforce loss %, engine internal component wear % (ICE/TC/MGU-K/Gearbox), DRS & ERS mechanical faults.
          3. *ERS & Power Unit:* Low battery reserve % warning, radiator dirty air water/oil overheating.
          ...
          6. *Rivals & DRS Threat:* Car behind inside DRS zone (<0.8–2.5s) with compound offset and damage notes, catching car ahead overtake directive.

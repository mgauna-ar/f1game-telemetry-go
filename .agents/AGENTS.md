...
*   **Data Transformation:** Keep frontend payloads lightweight. Only broadcast the specific data points needed by the UI (e.g., extracting `WorldPositionX/Z` for the mini-map) rather than sending raw, unparsed packets. For historical lap comparison telemetry, server-side LTTB downsampling (`?maxPoints=800`) is used to maintain 60FPS UI performance. Lap Comparator features a custom searchable session dropdown (with category filter pills and dedicated Sprint amber badges), 2-column layout with a sticky Track Map sidebar (with smart outward corner offsetting, anti-collision turn badges, view mode switcher, and turn quick-jump ribbon), separated Throttle/Brake charts with driver names, integrated ERS battery % and ERS deploy mode (Off, Medium, Hotlap, Overtake) step charts, synchronized track distance zoom buttons, real-time hover point telemetry readouts with contextual turn indication, and an embedded **AI Race Engineer** slide-over drawer with real-time SSE streaming for contextual telemetry analysis and coaching.

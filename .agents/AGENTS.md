...
*   **Navigation Layout:** Top nav bar features the `<F1TelemetryLogo />` brand identity, followed by three main sections:
    ...
    **3) Live Session**
    ...
    - **Interactive Voice Race Engineer** (`<LiveRadioHUD />` — floating bottom-left):
        ...
        - **Proactive Telemetry Radio Watcher & Session Intelligence** (`useProactiveTelemetryRadio`): Automatically adapts pit wall radio protocol by session type (Qualifying vs Free Practice vs Race). In **Qualifying / Shootouts**: alerts on out-lap traffic clean air gaps (<4s warning in sector 3), immediate lap invalidations (`CurrentLapInvalid` track limits), session clock countdowns (<3 min remaining for final run), and elimination danger zone warnings (P16 in Q1 / P11 in Q2). In **Race**: monitors tyre wear % warnings & punctures, thermal overheating (>115°C) & cold tyres, rival DRS threats with compound & damage context, and pit stop window / rival undercut attempts. In **All Sessions**: monitors Red Flags and rain radar crossover windows while strictly suppressing race battles in Qualy and Practice.

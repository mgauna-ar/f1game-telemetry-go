...
*   **Navigation Layout:** Top nav bar features the `<F1TelemetryLogo />` brand identity, followed by three main sections:
    ...
    **3) Live Session**
    ...
    - **Live Race Control Hub (2×2 grid):** Real-time incident feed, weather radar timeline, pit strategy matrix, live sector performance tracker.
        ...
        - **Dynamic Emotion & Prosody Modulation:** Telemetry-driven stress/urgency modulation dynamically accelerates speech rate and raises neural voice pitch during safety-critical moments (punctures, heavy damage, SC/VSC restarts, severe penalties), returning to calm cadence during standard cruising.
        - **Server-Side Telemetry Insight Engine & Proactive Radio Watcher** (`internal/api/insights.go` & `useProactiveTelemetryRadio`): Monitors 9 distinct telemetry subsystems with year-aware logic (2025 DRS vs 2026 Active Aero & Boost/Override), independent per-category anti-spam cooldowns, backend synthetic insight broadcasting (`PacketIDInsight` 250), and instant Emergency Bypass for safety-critical events:
          ...
          5. *Fuel & Predictive Pit Strategy:* Target deficit delta (laps) with Lift & Coast directive, server-side traffic window gap calculation on potential pit stop rejoin, rival undercut/overcut threat.
          ...
          7. *Micro-Sector Coaching & Teammate Context:* Server-side delta loss tracking across micro-sectors vs personal best, teammate position/pace comparisons, and team orders advisories.
          8. *Qualifying & Shootout Intelligence:* Out-lap clean air traffic gap warning (<4s in sector 3), flying lap track limits invalidation, session countdown timer (<3 min), Q1/Q2 elimination danger zone alerts.
          9. *Flags, Dynamic Weather & Race Control:* Full SC, Virtual Safety Car, Red Flag session halts, dynamic rain horizon and track grip transition alerts, corner cutting warnings before penalty count, steward penalties.

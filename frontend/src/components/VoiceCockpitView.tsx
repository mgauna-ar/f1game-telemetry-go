...
const HeroWaveformCanvas: React.FC<{ radioState: 'transmitting' | 'speaking' | 'idle' | 'processing' }> = ({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const barWidth = Math.max(3, (canvas.width - totalGapWidth) / barCount);
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        const barHeight = Math.max(4, norm * canvas.height * 0.92);
  ...
  if (!hasCanvasCtx) {
    return (
      <div className="voice-cockpit-eq-fallback">
  ...
  return (
    <canvas
      ...
      data-testid="voice-cockpit-waveform"
    />
  );
};
...
export const VoiceCockpitView: React.FC<VoiceCockpitViewProps> = ({
  ...
  return (
    <div className="voice-cockpit-container" data-testid="voice-cockpit-container">
      ...
      <div className={`voice-cockpit-hero-card ${stateClass}`}>
        <div className="voice-cockpit-hero-header">
          ...
          <div className="voice-cockpit-waveform-box">
            <HeroWaveformCanvas radioState={radio.isRadioEnabled ? radio.radioState : 'idle'} />
          </div>
      ...
      {/* Minimalist Vitals Telemetry Grid */}
      <div className="voice-cockpit-vitals-grid">

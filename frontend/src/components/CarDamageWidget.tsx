...
export const CarDamageWidget: React.FC<CarDamageWidgetProps> = ({ carDamage, driverName }) => {
  ...
  const tyres = [
    { key: 'FL', name: 'Front Left', wear: carDamage.TyresWear[2] ?? 0, damage: carDamage.TyresDamage[2] ?? 0, brakeDamage: carDamage.BrakesDamage[2] ?? 0 },
    { key: 'FR', name: 'Front Right', wear: carDamage.TyresWear[3] ?? 0, damage: carDamage.TyresDamage[3] ?? 0, brakeDamage: carDamage.BrakesDamage[3] ?? 0 },
    { key: 'RL', name: 'Rear Left', wear: carDamage.TyresWear[0] ?? 0, damage: carDamage.TyresDamage[0] ?? 0, brakeDamage: carDamage.BrakesDamage[0] ?? 0 },
    { key: 'RR', name: 'Rear Right', wear: carDamage.TyresWear[1] ?? 0, damage: carDamage.TyresDamage[1] ?? 0, brakeDamage: carDamage.BrakesDamage[1] ?? 0 },
  ];
  ...
  const aeroItems = [
    { label: 'FL Wing', val: carDamage.FrontLeftWingDamage },
    { label: 'FR Wing', val: carDamage.FrontRightWingDamage },
    { label: 'Rear Wing', val: carDamage.RearWingDamage },
    { label: 'Floor', val: carDamage.FloorDamage },
    { label: 'Diffuser', val: carDamage.DiffuserDamage },
    { label: 'Sidepod', val: carDamage.SidepodDamage },
  ];

  const engineItems = [
    { label: 'Engine', val: carDamage.EngineDamage },
    { label: 'Gearbox', val: carDamage.GearBoxDamage },
    { label: 'ICE', val: carDamage.EngineICEWear },
    { label: 'TC (Turbo)', val: carDamage.EngineTCWear },
    { label: 'MGU-K', val: carDamage.EngineMGUKWear },
    { label: 'MGU-H', val: carDamage.EngineMGUHWear },
    { label: 'Energy Store', val: carDamage.EngineESWear },
    { label: 'Control Elect.', val: carDamage.EngineCEWear },
  ];
  ...
  return (
    <div className="glass-panel" style={{ padding: '1.25rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '1rem' }}>
        {/* Left Column: 4 Tyres Grid (Span 6) */}
        <div style={{ gridColumn: 'span 6', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
            {tyres.map((tyre) => {
              ...
              return (
                <div key={tyre.key} style={{
                  padding: '8px 10px',
                  borderRadius: '6px',
                  backgroundColor: 'rgba(255, 255, 255, 0.03)',
                  border: `1px solid ${wearVal > 50 ? wearColor : 'rgba(255, 255, 255, 0.08)'}`,
                  display: 'flex',
        ...
        {/* Right Column: Aero & Powertrain Wear (Span 6) */}
        <div style={{ gridColumn: 'span 6', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {/* Aero Section */}
          <div>

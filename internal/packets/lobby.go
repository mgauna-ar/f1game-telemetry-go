...
func DecodeLobbyInfo(data []byte) (*PacketLobbyInfoData, error) {
	...
	itemSize := PerCarItemSize(playersPayload, header, structSize, 0)
	...
	for i := 0; i < numToRead && i < MaxCars; i++ {
		offset := i * itemSize
		if offset+structSize > len(playersPayload) {
			break
		}

		playerBytes := playersPayload[offset : offset+structSize]

...
// PacketHeader is the 29-byte header present at the start of every UDP packet.
...
func DecodeHeader(data []byte) (PacketHeader, error) {
	if len(data) < HeaderSize {
		return PacketHeader{}, fmt.Errorf("data too short for header: got %d bytes, need %d", len(data), HeaderSize)
	}
	var h PacketHeader
	err := binary.Read(bytes.NewReader(data[:HeaderSize]), binary.LittleEndian, &h)
	if err != nil {
		return PacketHeader{}, fmt.Errorf("failed to decode header: %w", err)
	}
	return h, nil
}

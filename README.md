...
- **Session Explorer & Historical Deep Dive** — Comprehensive historical session hub with aggregate KPI metrics (Total Sessions, Laps, Most Visited Circuit), visual Card Grid vs compact Data Table switcher, multi-attribute filtering (Track, Session Type, Weather) and column sorting. Features a 3-tab session deep-dive experience:
  - 🏆 **Classification & Laps**: P1/P2/P3 podium cards, driver standings with purple/green sector timing highlights, theoretical best lap calculations (`min(S1) + min(S2) + min(S3)`), interactive tyre stint timelines with compound pills and lap counts, car setup inspector, and direct **"Slot A"** / **"Slot B"** buttons on every completed lap to instantly load it into the Lap Comparator.
